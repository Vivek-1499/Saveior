...
export const checkBudgetAlert = inngest.createFunction(
  ...
  { cron: "0 */6 * * *" }, //every 6 hours check the budget
  async ({ step }) => {
...
export const processRecurringTransaction = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    await step.run("processing-transaction", async () => {
      const transaction = await db.transaction.findUnique({
        ...
        include: {
          account: true, //include acc information
        },
      });
  ...
  async ({ step }) => {
    ...
    for (const user of users) {
      await step.run(`generate-reports-${user.id}`, async () => {
        ...
        const insights = await generateFinanialInsights(stats, monthName);

        await sendEmail({
          to: user.email,
          subject: `Hey ${user.name} Your Monthly Financial Report for - ${monthName} is here`,
          react: EmailTemplate({
            userName: user.name,
            type: "monthly-report",
            data: {
...
async function generateFinanialInsights(stats, month) {
